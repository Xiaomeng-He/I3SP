...
def add_soc_eoc(df,
    ...
    Create rows for SOC (Start of Case) and EOC (End of Case). 
    ...
    """
    # sort the event log by case_id and timestamp
    ...
    for i in range(len(df)):
        ...
        if i == 0 or df[case_id].iloc[i] != df[case_id].iloc[i - 1]:
            ...
            # create a new row with 'SOC' as the event name, duplicating values 
            # from the first event in other columns
            soc_row = df.iloc[i].copy()
        ...
        # if this is the last event in a case, append a eoc_row after all events 
        # of the case
        if i == len(df) - 1 or df[case_id].iloc[i] != df[case_id].iloc[i + 1]:
            # create a new row with 'SOC' as the event name, duplicating values 
            # from the last event in other columns
            eoc_row = df.iloc[i].copy()
    ...
    # above the first event in a case, and EOC is always immediately below the 
    # last event in a case
